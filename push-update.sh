...
updates_dir=/data/tpp_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have ThePixelProject-VERSION-DATE-TYPE-* as format"
...
# Assume ThePixelProject-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.tpp.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.tpp.updater/databases/updates.db" \
